...
const checkGroceryItems = async () => {
    try {
        ...
        const shoppingListItems = await knex("grocery_items")
            .select("id", "user_id", "item_name", "quantity", "status", "expiration_date", "threshold_alert")
            .where(function () {
        ...
        if (shoppingListItems.length > 0) {
            ...
            shoppingListItems.forEach(item => {
                console.log(`- ${item.item_name} (Status: ${item.status}, Expiry: ${item.expiration_date})`);
            });
        } else {
        ...
        for (const item of shoppingListItems) {
            const { id, user_id, item_name, status, expiration_date, threshold_alert } = item;
            let alertMessage = "";
